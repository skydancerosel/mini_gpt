...
Controlled variation of AdamW's second-moment parameter $\beta_2$ produces
smooth degradation of backbone dominance, confirming that the structure is
optimizer-induced rather than loss-landscape-determined.
...
% ══════════════════════════════════════════════════════════════════════════════
\section{$\beta_2$ Controls Backbone Geometry}\label{sec:beta2}

If the backbone direction emerges from optimizer-integrated gradient history
rather than instantaneous gradient alignment or curvature structure, then
altering the optimizer's second-moment normalization should systematically
change the trajectory geometry.  We test this by varying $\beta_2$ in AdamW
while holding all other hyperparameters fixed and measuring the resulting drift
structure.

\subsection{Experimental Setup}\label{sec:beta2-setup}

We train the same architecture (8-layer, $d=512$, 51M parameters) on the same
dataset and seed under five values of $\beta_2 \in
\{0.99, 0.95, 0.90, 0.80, 0.0\}$, with all other hyperparameters fixed:
$\eta = 10^{-3}$, weight decay $= 0.5$, $\lambda = 2.0$, cosine schedule with
warmup.  Each run lasts 4{,}000 steps.  The purpose is not to optimize
performance but to probe geometric structure under controlled changes to
second-moment memory.

\subsection{Geometric Results}\label{sec:beta2-geometry}

\begin{table}[t]
\centering
\caption{Backbone geometry under $\beta_2$ variation.  PCA is computed on
         row-normalized trunk displacements over the window $[600, 2000]$.
         Update alignment is $\abs{\cos(\mathbf{u}_t, \vb)}$ averaged over
         six time points.  Gradient interference is
         $\cos(\mathbf{g}_{\mathrm{LM}}, \mathbf{g}_{\mathrm{probe}})$ at
         step~1600.  $\beta_2 = 0$ diverged (val loss $> 10^8$).}
\label{tab:beta2-geometry}
\begin{tabular}{@{}cccccc@{}}
\toprule
$\beta_2$ & PC1 (\%) & $k_{95}$ & Drift &
$\abs{\cos(\mathbf{u}, \vb)}$ & Interference \\
\midrule
0.99 & 68.1 & 6 &  106 & 0.226 & 0.022 \\
0.95 & 68.4 & 6 &  109 & 0.225 & 0.011 \\
0.90 & 66.3 & 7 &  113 & 0.220 & 0.040 \\
0.80 & 63.4 & 8 &  128 & 0.214 & 0.048 \\
0.0  & 51.6 & 7 & $2.1 \times 10^5$ & 0.099 & 0.273 \\
\bottomrule
\end{tabular}
\end{table}

As $\beta_2$ decreases (\Cref{tab:beta2-geometry}):

\begin{enumerate}
    \item Backbone dominance decreases smoothly
          (PC1: $68 \to 63\%$).
    \item Effective trajectory dimension increases
          ($k_{95}$: $6 \to 8$).
    \item Drift magnitude increases ($106 \to 128$),
          indicating less directionally constrained displacement.
    \item Update--backbone alignment weakens
          ($\abs{\cos}$: $0.226 \to 0.214$).
    \item $\beta_2 = 0$ diverges catastrophically, producing
          unstructured, high-norm drift.
\end{enumerate}

These results indicate that second-moment normalization constrains
optimization into a low-dimensional cumulative drift, suppressing transverse
wandering.

\subsection{Interpretation: Why $\beta_2$ Matters}\label{sec:beta2-mechanism}

Recall the AdamW update (\cref{eq:adamw-update}):
$\mathbf{u}_t = -\alpha\,\mathbf{m}_t / (\sqrt{\mathbf{v}_t} + \epsilon)$,
where $\mathbf{v}_t = \beta_2\,\mathbf{v}_{t-1} + (1-\beta_2)\,\mathbf{g}_t^2$
is the exponential moving average of squared gradients.  The parameter
$\beta_2$ controls how slowly $\mathbf{v}_t$ evolves.

Higher $\beta_2$ stabilizes per-parameter scaling, reduces stochastic
anisotropy in the denominator, and enforces consistent directional integration
across steps.  Lower $\beta_2$ increases variability in scaling, amplifies
coordinate-wise noise, and allows the trajectory to wander in more directions.

The backbone direction appears when accumulated updates remain coherently
aligned over long horizons; $\beta_2$ controls this coherence.

\subsection{Relation to SGD}\label{sec:beta2-sgd}

The SGD-family controls in \Cref{sec:sgd-control} showed that momentum alone
produces a degenerate one-dimensional trajectory (PC1 $\approx 100\%$,
$k_{95} = 1$).  The $\beta_2$ ablation complements this: reducing adaptive
scaling within AdamW degrades but does not eliminate multi-dimensional
structure, while removing it entirely (SGD) collapses it.  This suggests that
adaptive second-moment scaling, not momentum alone, is critical for the
observed structure.

\subsection{Functional Consequence: Reheating Recoverability}\label{sec:beta2-reheat}

A downstream readout of backbone geometry is the ability to re-excite the
transverse probe mode via reheating (\Cref{sec:reheating}).  We apply the same
reheating protocol ($\lambda = 4.0$, fresh optimizer, 2{,}000 steps,
$\eta = 6 \times 10^{-4}$) to checkpoints from the $\beta_2$ runs
(\Cref{tab:beta2-reheat}).

\begin{table}[t]
\centering
\caption{Best reheated $p_{\mathrm{ood}}$ under $\beta_2$ variation.
         Reheating uses $\eta = 6 \times 10^{-4}$, $\lambda = 4.0$,
         2{,}000 steps from each checkpoint.}
\label{tab:beta2-reheat}
\begin{tabular}{@{}cccc@{}}
\toprule
$\beta_2$ & Checkpoint & Best $p_{\mathrm{ood}}$ & Re-entry gain $G$ \\
\midrule
0.95 & 2000 & 0.672 & $+0.000$ \\
0.95 & 4000 & 0.421 & $+0.200$ \\
0.80 & 2000 & 0.191 & $+0.002$ \\
0.80 & 4000 & 0.131 & $+0.006$ \\
\bottomrule
\end{tabular}
\end{table}

Lower $\beta_2$ weakens the transverse probe mode and reduces re-excitation
under identical reheating.  At $\beta_2 = 0.95$, the probe basin at step~2000
is deep enough that reheating recovers to the original peak; by step~4000,
recovery is partial.  At $\beta_2 = 0.80$, the basin is effectively flat at
both checkpoints.  This confirms that $\beta_2$ controls not only static
trajectory geometry but also the dynamical accessibility of transverse modes.

\subsection{Summary}

Across controlled $\beta_2$ variations, we observe a smooth degradation of
backbone dominance, trajectory complexity, and reheating recoverability.
These results support the hypothesis that the backbone is optimizer-induced,
emerging from second-moment stabilization of cumulative update directions
rather than instantaneous gradient alignment or local curvature structure.
...
The $\beta_2$ ablation (\Cref{sec:beta2}) provides a finer-grained view:
within AdamW, reducing $\beta_2$ smoothly degrades backbone dominance, effective
dimensionality, and update--backbone alignment, confirming that second-moment
memory is the specific mechanism controlling trajectory geometry.
